// Power Apps nhận một file TSX typed duy nhất. Source có thể chỉnh sửa nằm trong
// src/; script này flatten import có tên nhưng vẫn giữ metadata của RuntimeTypes.
...
function claim(name, identity) {
    // Đảm bảo mỗi identifier chỉ xuất hiện một lần sau khi module được ghép phẳng.
...
function visit(file) {
    // visiting phát hiện vòng lặp import; visited tránh xử lý một module hai lần.
    ...
    for (const node of source.statements) {
        ...
        // Giữ comment và type declaration; chỉ bỏ keyword export sau khi đã kiểm tra
        // trùng identifier để artifact cuối vẫn là một module hợp lệ.
...
visit(entry);
// Sắp xếp import ổn định để kết quả build không thay đổi giữa các lần chạy.
...
if (process.argv.includes('--check')) {
    // Git có thể checkout artifact bằng CRLF trên Windows nên chuẩn hóa newline trước khi so sánh.
