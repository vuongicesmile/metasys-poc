...
`FMCentralBms` solution, both tables, columns, alternate key, and runtime role
were provisioned. The initial SQL backlog was delivered with zero dead-letter
rows, and 25 retained history samples were reconciled successfully. The exported
...
- Security role `FM Central BMS Integration`, with organization-level Create,
  Read and Write on these two tables only.

Assign the runtime application user that role. If provisioning used the same app,
remove its deployment/admin roles before starting continuous synchronization.
