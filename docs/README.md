...
| Plan | [Refactor Fake BMS services](plans/fake-bms-services-refactor.vi.md) | Tách shared contracts, fixture, mutable point store, simulator, subscription và SSE endpoint |
...
| Plan | [SharePoint Online → Dataverse file ingestion](plans/sharepoint-to-dataverse-file-ingestion.vi.md) | Thiết kế dùng SharePoint/Dataverse connectors, không tự viết Graph API, upload File column và parse tùy chọn |
...
| Bài thực hành đề xuất | [Plugin 02 — khóa Equipment Code sau khi tạo](plans/plugin-lock-equipment-code.vi.md) | Học Pre Image, so sánh mã cũ/mới, đăng ký Update step và test trên app; chưa deploy |
